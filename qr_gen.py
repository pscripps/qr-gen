...
def choose_version(data_bytes):
    ...
    for v in range(1, 41):
        ...
        if total_bytes_needed <= cap:
            return v
    raise ValueError(f"Data too large for QR ({n} bytes)")
...
def render_png(matrix, size, scale=10, border=4, filename="qrcode.png"):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 qr_gen.py '<data>' [output.png]")
        sys.exit(1)
    data = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else "qrcode.png"
    matrix, size = generate_qr(data)
    render_png(matrix, size, scale=10, border=4, filename=output)
